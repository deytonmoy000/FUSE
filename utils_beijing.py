...
def gen_granular_data(grid_size):
    ...
    df = pd.read_csv('data/data_beijing_updated.csv')

    df = df[~((df['Longitude'] > cur_max_LONG) | (df['Latitude'] > cur_max_LAT) | (df['Longitude'] < cur_min_LONG) | (df['Latitude'] < cur_min_LAT))]
...
#     df.to_csv('data/data_beijing_new.csv')

    fname = f'data/data_beijing_updated_{str(int(grid_size*1000))}m.csv'
...
#     print("SAVED DATAFRAME TO ", fname)
    ...
    return df, n_rows, n_cols, n_rows_valid, n_cols_valid

def create_grid_json_updated(grid_size=1000):
    grid_size /= 1000
    block_size = 0.01  # 1 km in degrees
    block_size *= grid_size
    min_lat = cur_min_LAT
    ...
    file_path = f"data/beijing_map_{str(int(grid_size*1000))}m.json"
...
def update_grids_info(df, grid_size):
    ...
    n_rows_valid, n_cols_valid = df.Lat_id.max(), df.Long_id.max()
...
def get_valid_grid(df, grid_size):
    grid_size *= 1000
    fname = f'data/data_beijing_updated_{str(int(grid_size))}m.csv'

    df = pd.read_csv(fname)
    
    df = df[~((df['Longitude'] > cur_max_LONG) | (df['Latitude'] > cur_max_LAT) | (df['Longitude'] < cur_min_LONG) | (df['Latitude'] < cur_min_LAT))]
    
    min_LONG_ID, min_LAT_ID = df.Long_id.min(), df.Lat_id.min()
    max_LONG_ID, max_LAT_ID = df.Long_id.max(), df.Lat_id.max()
    ...
    fname_valid_grids = f'data/map/visited_{str(int(grid_size))}m_beijing.txt'
    ...
    for lon in range(min_LONG_ID, max_LONG_ID):
        for lat in range(min_LAT_ID, max_LAT_ID):
            if lon == 252 or lat == 182:
                print(lon, lat)
            visited_cells.add((lon, lat))
            
    fname_valid_grids = f'data/map/valid_grids_{str(int(grid_size))}m_beijing.txt'
    with open(fname_valid_grids, "w") as file:
        for cell in visited_cells:
            x, y = cell
            line = str(x)+','+str(y)+'\n'
...
def csv_to_grid_updated(grid_size=1000):
    ...
    fname_json = f"data/beijing_map_{str(int(grid_size*1000))}m.json"
    df = pd.read_csv(fname)
...
#     print(df.head())
    ...
    fname_grids = f'data/map/grids_new_{str(int(grid_size*1000))}m_beijing.txt'
...
def grid_to_map_updated(grid_size):
    ...
    grid_ids = {}
    fname_grids = f'data/map/grids_new_{str(int(grid_size))}m_beijing.txt'
    ...
    print("GRIDS CREATED -- MAX LONG ID: {}({})  -- MAX LAT ID: {}({})".format(max_LONG_id, final_max_LONG, max_LAT_id, final_max_LAT))
    visited_cells = set()
    fname_valid_grids = f'data/map/visited_{str(int(grid_size))}m_beijing.txt'
    ...
    print(gridSize_info_dict[grid_size][0], gridSize_info_dict[grid_size][1])
    
    for cell in visited_cells:
        grid_x, grid_y = cell
        if grid_x >= gridSize_info_dict[grid_size][1] or grid_y >= gridSize_info_dict[grid_size][0]:
            print(grid_x, grid_y)
            continue
...
def raw_data_preprocessing_old():
    ...
    df.to_csv('data/data_beijing_updated.csv', header=True, index=False)

    print("SAVED DATAFRAME TO data/data_beijing_updated.csv")
...
def generate_neighbors_list(grid_size):
    ...
    visited_cells = set()
    visited_locs = set()
    with open(f'data/map/valid_grids_{str(int(grid_size))}m_beijing.txt', "r") as file:
        for line in file:
    ...
    with open(f'data/map/valid_grids_neighbors_{str(int(grid_size))}m_beijing.txt', 'w') as file:
...
def generate_neighbors_list_wBorders(n, grid_size, df, alg, tf, km_time):
    ...
    sorted_neighbors = {key: neighbors[key] for key in sorted(neighbors)}
    fname = f'data/map/valid_grids__neighbors_{str(int(grid_size))}m_beijing_{n}.txt'
...
def load_neighbors_list():
    ...
    neighbors = collections.defaultdict(list)
    with open('data/valid_grids_neighbors_beijing.txt', "r") as file:
        for line in file:
            pos, nei = line.split('|')
            pos = int(pos)
            nei = nei.split(',')
...
def generate_time_intervals(tf):
    ...
    start_date = datetime(2009, 1, 3, 0, 0)
    end_date = datetime(2009, 1, 4, 0, 0)  # Change this to the end date you need

    interval = timedelta(minutes=(tf-1))
    next_slot = timedelta(minutes=tf)
    current_date = start_date
    fname = 'data/time_intervals_beijing_'+str(tf)+'.txt'
    with open(fname, 'w') as file:
...
def get_initial_loc_DSVs(grid_size, nDSV=1, seed=42):
    ...
    fname = f'data/map/valid_grids_neighbors_{str(int(grid_size))}m_beijing_{nDSV}.txt'
